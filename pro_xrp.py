...
he_so = 1000 # If price too small, need to multiply with he_so to correct the "i" in loop below
XRP = 0.4 * he_so

# Load the dataset
data = pd.read_csv('data/XRP_15M.csv')
unit_price = round(XRP / 200) # 0.5% of current price
min_probability = 55 # input accept win rate %
num_candle = 3 # input number of candle same color
scale_price = list(range(unit_price, unit_price * 15, unit_price))
